...
Schema v4 receipts bind the current controller-computed implementation snapshot. The snapshot excludes controller-owned workflow artifacts so recording a receipt does not invalidate itself. Any later implementation digest change makes the receipt stale until the affected checks are rerun and rebound. A hook trust/configuration statement is `PASS` only after the installed host actually executes the trusted hook; package tests alone are not live evidence.

Enforcement violations record stable central rule IDs. PostToolUse audit reports side effects and marks state; it never claims that a completed tool action was undone.
