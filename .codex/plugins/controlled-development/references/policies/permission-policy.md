...
## Controller and hook enforcement

- For schema v4 workflows, `execution-policy.json` is the machine-readable task contract. Markdown never overrides it.
- The controller owns authorization and returns `{allowed, ruleId, reason, audit}`. Hook adapters only parse events and apply that decision.
- An active workflow must fail closed when state, policy, task, phase, path, command, or tool classification is missing or inconsistent.
- Protected controller paths include the active binding, workflow state, ledger events, and hook snapshot storage. Task write lists cannot grant direct mutation of these paths.
- No active change means `CD-E000-INACTIVE-NOOP`; it does not authorize a hidden or ambiguous active workflow.
- Disabled, untrusted, or unobserved hooks are not enforcement evidence and cannot support a `PASS` claim.
